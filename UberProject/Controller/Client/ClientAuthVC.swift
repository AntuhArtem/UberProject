...
class ClientAuthVC: UIViewController, UITextFieldDelegate {

    private let clientSegue = "ClientMapVC"
    ...
    @IBAction func signUp(_ sender: UIButton) {
        if emailTextField.text != "" && passwordTextField.text != "" {
            AuthProvider.Instance.signUp(withEmail: emailTextField.text!, password: passwordTextField.text!, loginHandler: {(message) in
                ...
                {
                    UberHandler.Instance.client = self.emailTextField.text!
                    
                    //if we logout, our text fields must be empty
                    self.emailTextField.text! = ""
                    self.passwordTextField.text! = ""
                    self.performSegue(withIdentifier: self.clientSegue, sender: nil)
                }
            })
        }
        else
